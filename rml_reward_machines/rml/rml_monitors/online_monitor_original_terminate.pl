...
log(Log) :-
    ...
    ;   Log=(TE,E,error),
            writeln(Stream,"Trace expression:"),writeln(Stream,TE),writeln(Stream,"Event: "),writeln(Stream,E),writeln(Stream,"Error")
    ),
    nl(Stream),
    flush_output(Stream);
    true.

% --- NEW: minimal initializer for explicit resets ---
initialize_monitor :-
...
manage_event(WebSocket) :-
    ...
    ;   E=Msg.data,
        ...
        ;   (   nb_setval(state, 0),
                writeln('verdict = False'),
                term_string(TE1, TE1Str),
                Reply=(_{}.put(E).put(_{verdict:false, spec:TE1Str}))      % Verdict = False
            )
        ),
        atom_json_dict(Json,Reply,[as(string)]),
        ws_send(WebSocket, string(Json)),
        % --- NEW: honor terminate flag from event to reset state ---
        (   get_dict(terminate, E, true)
        ->  writeln('terminate = true, reinitializing monitor'),
            initialize_monitor
        ;   true
        ),
        manage_event(WebSocket)
